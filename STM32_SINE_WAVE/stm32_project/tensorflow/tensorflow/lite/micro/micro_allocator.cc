...
MicroAllocator::MicroAllocator(TfLiteContext* context, const Model* model,
  ...
  if (status != kTfLiteOk) {
    ...
    active_ = false;
    print("\nmicro_allocator: 463: active_: false");
  } else {
    active_ = true;
    print("\nmicro_allocator: 466: active_: true");
  }
}
...
TfLiteStatus MicroAllocator::AllocateNodeAndRegistrations(
  ...
  if (output == nullptr) {
    ...
    print("micro_allocator: 478: failed to allocate the memory\n\n");
    return kTfLiteError;
  }
  ...
  for (size_t i = 0; i < subgraph_->operators()->size(); ++i) {
    ...
    if (index >= opcodes->size()) {
      ...
      print("micro_allocator: 500: Missing registration for opcode_index\n\n");
      return kTfLiteError;
    }
    ...
    if (status != kTfLiteOk) {
      ...
      print("micro_allocator: 510: returning status\n\n");
      return status;
    }
    ...
    if (registration == nullptr) {
      TF_LITE_REPORT_ERROR(error_reporter_, "Skipping op for opcode_index %d\n",
                           index);
      print("micro_allocator: 517: Skipping op for opcode_index\n\n");
      return kTfLiteError;
    }
    ...
    if (op_type != BuiltinOperator_CUSTOM && op->custom_options()) {
      ...
      print("micro_allocator: 529: found buildin operator\n\n");
      return kTfLiteError;
    }
  ...
  *node_and_registrations = output;
  print("micro_allocator: 560: return OK status\n\n");
  return kTfLiteOk;
}
